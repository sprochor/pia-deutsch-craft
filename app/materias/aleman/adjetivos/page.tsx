...
export default function AdjetivosPage() {
  ...
  const poolSituaciones = [
    ...
    { id: 6, texto: "Un Enderman te mira muy serio y no hace chistes.", opciones: ["witzig", "ernst"], correcta: "ernst" },
    { id: 7, texto: "Pia siempre hace sus tareas de alemán con ganas.", opciones: ["fleißig", "faul"], correcta: "fleißig" },
    { id: 8, texto: "Un zombie es muy lento y solo quiere dormir.", opciones: ["sportlich", "faul"], correcta: "faul" },
    { id: 9, texto: "Corres muy rápido por todos los biomas.", opciones: ["sportlich", "schüchtern"], correcta: "sportlich" },
    { id: 10, texto: "Escuchas con atención los problemas de un amigo.", opciones: ["verständnisvoll", "ungeschickt"], correcta: "verständnisvoll" }
  ];
  ...
  const comprobar = () => {
    const todasCorrectas = situaciones.every(s => respuestas[s.id] === s.correcta);
    if (todasCorrectas) {
      playSuccess();
  ...
  return (
    <main className="min-h-screen p-8 max-w-4xl mx-auto relative">
      ...
      <div className="mc-card mb-8 border-[#3b82f6]">
        ...
        <p className="text-gray-300 font-bold italic">Forja las características de cada mob...</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {situaciones.map((s) => {
          ...
          return (
            <div key={s.id} className="mc-card bg-[#3a3a3a] border-gray-600 p-4 flex flex-col justify-between">
                ...
                {s.opciones.map((opcion) => (
                  <button
                    ...
                    className={`mc-button !py-2 !text-sm text-white ${
                      ...
                      mostrarResultado && opcion === s.correcta ? '!bg-[#528044]' : ''
                    } ${
        ...
        {!mostrarResultado ? (
          <button 
            ...
            className={`mc-button text-white w-full py-4 text-2xl font-black ${
              Object.keys(respuestas).length < situaciones.length ? 'opacity-50 !bg-[#7a7a7a]' : '!bg-[#3b82f6]'
            }`}
          >
            FORJAR RESPUESTAS
          </button>
